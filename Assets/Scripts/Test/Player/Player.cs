...
using UnityEngine.UI; // Ajoute cette ligne au début
...
public class Player : MonoBehaviour
{
    ...
    private float pushDuration = 0.18f; // Durée de l'effet, à ajuster
    ...
    public void DeployFriend()
    {
        if (friendInstance != null && !isFriendDeployed && !disableDeployFriend)
        {
            friendInstance.gameObject.SetActive(true);
    ...
    public void RecallFriend()
    {
        ...
        {
            // Va chercher le script Friend
            Friend friendScript = friendInstance.GetComponent<Friend>();
            if (friendScript != null && friendScript.IsBonusActive())
            {
                Debug.Log("Impossible de rappeler le Friend pendant un bonus d'énergie.");
                // Tu peux ici afficher un message à l’écran ou jouer un son
                return;
            }

            // Le rappel normal, autorisé si aucun bonus
            friendInstance.SetActive(false);
    ...
    void Control()
    {
        ...
        // Contrôle horizontal classique
        if (Mathf.Abs(moveInput) > 0.01f)
        {
            // Si le joueur n'a pas (une vitesse > moveSpeed et va dans le même sens que l'input)
            if (
                !(
                    Mathf.Sign(moveInput) == Mathf.Sign(currentVx)
                    && Mathf.Abs(currentVx) > moveSpeed
                )
            )
            {
    ...
    public void ShowDamageScreen()
    {
        ...
        damageScreen.color = new Color(1f, 0f, 0f, 0.3f); // Rouge transparent
